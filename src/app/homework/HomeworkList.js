				...
				<div style={{display: 'flex', justifyContent: 'center'}}>
					<ItemListPage width="1200px"
						...
						tableHeaders={[
							...
							<TableCell key="homework-name">Homework Name</TableCell>,
							<TableCell key="problem-ids">Problem IDs</TableCell>,
							<EmptyCell key="empty"/>
						]}
						tableRowGenerator={{
							list: homeworkList,
							...
							data: homework => [
								<TableCell key={homework.id}>{homework.id}</TableCell>,
								<Link key={`${homework.id}-link`} to={`/homework/${homework.id}/homework`}
									onClick={() => setCurrentHomework(homework)}>
									{homework.name}</Link>,
								<TableCell key={`${homework.id}-problemIds`}>{homework.problemIds.join(', ')}</TableCell>,
								<TableCell key={`${homework.id}-btn`}>{actionItemsButton({homework})}</TableCell>,
							]
						}}/>
